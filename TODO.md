...
- [x] `PermittedSubclasses` (JVMS 4.7.31): emitted on an enum with constant
      bodies (implicitly sealed over its `E$N`), and on a `sealed` class or
      interface - from its explicit `permits A, B` clause when present, otherwise
      (implicit permits) from the direct subclasses declared in the same
      compilation unit, both in source order (matching javac). `sealed` carries no
      class flag of its own.
