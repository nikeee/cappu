...
function parseExpectedToken<T extends Node>(kind: SyntaxKind, message?: DiagnosticMessage): T {
  return (
    ...
    createMissingNode<T>(
      ...
      message ?? Diagnostics._0_expected,
      tokenToString(kind) ?? "",
    )
  );
}
...
function isStartOfParameter(): boolean {
  return token() === SyntaxKind.AtToken || token() === SyntaxKind.FinalKeyword || isStartOfType();
}
...
function parseEntityName(): EntityName {
  ...
  // Only consume a dot that is followed by an identifier, so a ".<T>" generic
  // method invocation is left for the expression parser rather than eaten here.
  while (
    token() === SyntaxKind.DotToken &&
    lookAhead(() => (nextToken(), token() === SyntaxKind.Identifier))
  ) {
...
function parseModifiers(): NodeArray<ModifierLike> | undefined {
  ...
  while (true) {
    ...
    if (
      isContextualKeyword("sealed") &&
      lookAhead(() => (nextToken(), isContextualModifierFollow()))
    ) {
...
function parseClassMember(): Node {
  ...
  if (
    token() === SyntaxKind.Identifier &&
    lookAhead(() => (nextToken(), token() === SyntaxKind.OpenBraceToken))
  ) {
...
function parseEnumBody(): {
  enumConstants: NodeArray<EnumConstantDeclaration>;
...
function parseCompactConstructor(
  ...
  const name = parseIdentifier();
  const body = parseBlock();
  const node = createNode<CompactConstructorDeclaration>(
...
function parseTypeDeclaration(): Statement {
  ...
  switch (token()) {
    ...
    default: {
      ...
      return createMissingNode<Statement>(
...
function parseModuleDirective(): Node {
  ...
  if (isContextualKeyword("exports"))
    return parseExportsOrOpensDirective("exports", SyntaxKind.ExportsDirective);
  if (isContextualKeyword("opens"))
    return parseExportsOrOpensDirective("opens", SyntaxKind.OpensDirective);
...
function parseLambdaParameter(): Node {
  ...
  if (
    ...
    lookAhead(
      () => (
        nextToken(),
        token() === SyntaxKind.CommaToken || token() === SyntaxKind.CloseParenToken
      ),
    )
  ) {
...
function parseAtom(): Expression {
  ...
  switch (token()) {
    ...
    default:
      ...
      return createMissingNode<Expression>(
        SyntaxKind.Identifier,
        /*reportAtCurrentPosition*/ false,
...
function parseExpressionSuffixes(start: Expression): Expression {
  ...
  while (true) {
    ...
    if (token() === SyntaxKind.ColonColonToken) {
      ...
      nextToken();
      const typeArguments = token() === SyntaxKind.LessThanToken ? parseTypeArguments() : undefined;
      const node = createNode<MethodReferenceExpression>(
...
function parseArrayInitializer(): ArrayInitializer {
  ...
  const elements = parseDelimitedList(
    ParsingContext.ArrayInitializerElements,
    parseArrayInitializerElement,
  );
  parseExpected(SyntaxKind.CloseBraceToken);
  const node = createNode<ArrayInitializer>(SyntaxKind.ArrayInitializer, pos);
...
function parseStatement(): Statement {
  ...
  if (
    token() === SyntaxKind.Identifier &&
    lookAhead(() => (nextToken(), token() === SyntaxKind.ColonToken))
  ) {
  ...
  if (
    isModifierKeyword(token()) ||
    token() === SyntaxKind.AtToken ||
    isLocalVariableDeclarationStart()
  ) {
...
function parseLocalVariableDeclarationRest(
  ...
  const node = createNode<LocalVariableDeclarationStatement>(
...
function parseTryStatement(): TryStatement {
  ...
  const resources =
    token() === SyntaxKind.OpenParenToken ? parseResourceSpecification() : undefined;
...
export function parseSourceFile(fileNameArg: string, text: string): SourceFile {
  ...
  parsingContext = 0;
  parseErrorBeforeNextFinishedNode = false;
  scanner = createScanner(text, (message, errPos, length) =>
    parseErrorAtPosition(errPos, length, message),
  );
  ...
  const endOfFileToken = parseExpectedToken<Token<SyntaxKind.EndOfFileToken>>(
  ...
  switch (node.kind) {
    ...
    case SyntaxKind.ClassDeclaration: {
      ...
      return (
        ...
        visitNodes(cbNode, cbNodes, n.permitsTypes) ||
