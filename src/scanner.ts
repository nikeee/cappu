...
import { type DiagnosticMessage, type ErrorCallback, type Scanner, SyntaxKind, TokenFlags } from "./types.ts";
...
export function createScanner(textInitial = "", onErrorInitial?: ErrorCallback): Scanner {
	...
	function scanStringOrTextBlock(): SyntaxKind {
		...
		if (text.charCodeAt(pos + 1) === Char.DoubleQuote && text.charCodeAt(pos + 2) === Char.DoubleQuote) {
