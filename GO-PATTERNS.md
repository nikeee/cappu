...
|---|---|---|
| `comment-json` (round-trip edit) | shared hand-rolled span editor (`internal/config/edit.go`, reverse-ported to `src/cli/jsoncEdit.ts`) + `tidwall/jsonc` (read) | done (2026-07: replaced sjson; both builds edit byte-identically) |
| `zod` (validate + JSON-Schema gen) | `encoding/json` + struct defaults + hand validation; schema is the checked-in zod output (`internal/config/cappu.schema.json`, go:embed, `node --run schema:write` regenerates) | done (2026-07: replaced invopop reflection for byte parity) |
...
## Reverse ports: shared byte-exact algorithms (jsoncEdit)

Usually the TS build is the reference and Go mirrors it. When the GO side has
the better design, port it back: the surgical JSONC editor started as Go's
sjson usage, but sjson's insert formatting (compact glue, no indent inference)
could never match a hand-formatted file, so a shared span-editing algorithm was
written once and implemented twice - `src/cli/jsoncEdit.ts` (reference) and
`internal/config/edit.go` (port, with a `// Port of src/cli/jsoncEdit.ts`
header). The contract is byte-identical output for the same edit, enforced by
mirrored test files (`src/cli/jsoncEdit.test.ts` <-> `internal/config/edit_test.go`)
whose cases are copied verbatim. Same pattern for generated artifacts: rather
than reimplementing zod's JSON-Schema generation, the generated file is checked
in (`internal/config/cappu.schema.json`), embedded via go:embed, regenerated by
`node --run schema:write`, and drift-guarded by a TS test that regenerates and
diffs. Exempt such byte-exact fixtures from formatters (.oxfmtrc.json
ignorePatterns) - oxfmt rewriting the schema was caught by the drift test.

## UTF-16 strings (JS) vs UTF-8 strings (Go): class-file text

A JS string is a sequence of UTF-16 code units and can hold an unpaired
surrogate; a Go string is bytes and `[]rune`/`string(rune)` silently turns a
surrogate into U+FFFD. Class files carry modified UTF-8 (JVMS 4.4.7), where a
supplementary character is stored *as* a surrogate pair, so the two builds only
...
The same asymmetry bites Java `\uXXXX` escapes in *source* text: cappu's Go
lexer materializes them into a Go string, so a surrogate-pair escape
(`"😀"`) collapses to two U+FFFDs while the TS lexer keeps it. Write
such test fixtures with the literal character, not the escape pair.

## Java number formatting (Float.toString / Double.toString)

`javap` prints float and double constants exactly as `Float.toString` /
`Double.toString` would, and neither `strconv.FormatFloat` nor JS
`Number.prototype.toString` matches it. Both builds implement the same search
(`shortestDigits` in `disasm.ts` / `disasm.go`): take 40 exact significant
digits, then for each length try *both* bracketing candidates (truncate and
truncate+1), keep one that parses back to the same value, and on a tie prefer
the digit-even one. Two Java-specific rules on top: the plain/scientific
switch is at `1e-3`/`1e7` (not JS's `1e21` or Go's `%g` heuristics), and a
